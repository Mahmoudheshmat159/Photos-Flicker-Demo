...
extension NSMutableAttributedString {
    ...
    func underlineColor(color: UIColor?, font: UIFont?) {
        ...
        addAttributes(attributes, range: NSMakeRange(0, self.length))
    }
}
...
extension String {
    ...
    func changeWordFont(_ textToFind: String?, with font: UIFont) -> NSMutableAttributedString {
        let string = NSMutableAttributedString(string: self)
        string.setFontForText(textToFind, with: font)
        return string
    }
}
...
public extension String {
    ...
    var width: CGFloat {
        let string = NSAttributedString(string: "       "+self+"       ")
        return string.size().width+10
    }
    var limit: String {
        if self.count > 30 {
            let string = self.prefix(30)
