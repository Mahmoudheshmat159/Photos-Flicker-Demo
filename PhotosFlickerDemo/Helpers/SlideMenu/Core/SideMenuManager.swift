...
open class SideMenuManager: NSObject {
    ...
    internal static var appScreenRect: CGRect {
        let appWindowRect = UIApplication.keyWindow?.bounds ?? UIWindow().bounds
    ...
    /**
     ...
     - popWhenPossible: If a view controller already in the stack is of the same class as the pushed view controller,
     the stack is instead popped back to the existing view controller.
     This behavior can help users from getting lost in a deep navigation stack.
     - preserve: If a view controller already in the stack is of the same class as the pushed view controller,
     the existing view controller is pushed to the end of the stack. This behavior is similar to a UITabBarController.
     - preserveAndHideBackButton: Same as .preserve and back buttons are automatically hidden.
     - replace: Any existing view controllers are released from the stack and replaced with the pushed view controller.
     Back buttons are automatically hidden. This behavior is ideal if view controllers require a lot of memory or their state doesn't need to be preserved..
     - subMenu: Unlike all other behaviors that push using the menu's presentingViewController,
     this behavior pushes view controllers within the menu.  Use this behavior if you want to display a sub menu.
     */
    ...
    /**
     Width of the menu when presented on screen, showing the existing view controller in the remaining space.
     //Default is 75% of the screen width or 240 points, whichever is smaller.
    ...
    /// The amount to scale the existing view controller or the menu view controller depending on the `menuPresentMode`.
    //Default is 1 for no scaling. Less than 1 will shrink, greater than 1 will grow.
    open var menuAnimationTransformScaleFactor: CGFloat = 1

    /// The background color behind menu animations. Depending on the animation settings this may not be visible.
    ...
    /// Enable or disable interaction with the presenting view controller while the menu is displayed. Enabling may make it difficult
    //to dismiss the menu or cause exceptions if the user tries to present and already presented menu. Default is false.
    open var menuPresentingControllerUserInteraction: Bool = false
    ...
    fileprivate func setupNavigationController(_ forMenu: UISideMenuNavigationController?, leftSide: Bool) {
        ...
        if forMenu.sideMenuManager != self {
            ...
            if forMenu.sideMenuManager?.menuLeftNavigationController == forMenu {
                //print("SideMenu Warning: \(String(describing: forMenu.self)) was already assigned to the menuLeftNavigationController of
                //\(String(describing: forMenu.sideMenuManager!.self)). When using multiple SideMenuManagers you may want to use new instances
                //of UISideMenuNavigationController instead of existing instances to avoid crashes if the menu is presented more than once.")
            } else if forMenu.sideMenuManager?.menuRightNavigationController == forMenu {
                //print("SideMenu Warning: \(String(describing: forMenu.self)) was already assigned to the menuRightNavigationController
                //of \(String(describing: forMenu.sideMenuManager!.self)). When using multiple SideMenuManagers you may want to use
                //new instances of UISideMenuNavigationController instead of existing instances to avoid crashes if the menu is presented more than once.")
            }
            #endif
    ...
    @discardableResult open func menuAddScreenEdgePanGesturesToPresent(toView: UIView, forMenu: UIRectEdge? = nil) -> [UIScreenEdgePanGestureRecognizer] {
        ...
        if forMenu != .right {
            ...
            if menuLeftNavigationController == nil {
                //print("SideMenu Warning: menuAddScreenEdgePanGesturesToPresent was called before menuLeftNavigationController was set.
                //The gesture will not work without a menu. Use menuAddScreenEdgePanGesturesToPresent(toView:forMenu:) to add gestures for only one menu.")
            }
            #endif
        }
        if forMenu != .left {
            ...
            if menuRightNavigationController == nil {
                //print("SideMenu Warning: menuAddScreenEdgePanGesturesToPresent was called before menuRightNavigationController was set.
                //The gesture will not work without a menu. Use menuAddScreenEdgePanGesturesToPresent(toView:forMenu:) to add gestures for only one menu.")
            }
            #endif
        }
    ...
    @discardableResult open func menuAddPanGestureToPresent(toView: UIView) -> UIPanGestureRecognizer {
        ...
        if menuLeftNavigationController ?? menuRightNavigationController == nil {
            //print("SideMenu Warning: menuAddPanGestureToPresent called before
            //menuLeftNavigationController or menuRightNavigationController have been defined. Gestures will not work without a menu.")
        }
