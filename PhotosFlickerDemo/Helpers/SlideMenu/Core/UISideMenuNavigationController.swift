...
open class UISideMenuNavigationController: UINavigationController {
    ...
    /// Delegate for receiving appear and disappear related events. If `nil` the visible view controller that displays
    //a `UISideMenuNavigationController` automatically receives these events.
    ...
    //Default is zero. When zero, `sideMenuManager.menuWidth` is used. This property cannot be changed while the
    //isHidden property is false.
    ...
    override open func viewDidAppear(_ animated: Bool) {
        ...
        if topViewController == nil {
            //print("SideMenu Warning: the menu doesn't have a view controller to show! UISideMenuNavigationController
                //needs a view controller to display just like a UINavigationController.")
        }
        #endif
    }
    ...
    override open func viewWillTransition(to size: CGSize, with coordinator: UIViewControllerTransitionCoordinator) {
        ...
        coordinator.animate(alongsideTransition: { (_) in
            ...
            NotificationCenter.default.addObserver(self.transition, selector: #selector(SideMenuTransition.handleNotification),
                                                   name: UIApplication.willChangeStatusBarFrameNotification, object: nil)
        })
    }

    func pushMenuController(_ viewController: UIViewController, animated: Bool) {
        ...
        guard let navigationController = potentialNavigationController as? UINavigationController else {
            //print("SideMenu Warning: attempt to push a View Controller from \(String(describing: potentialNavigationController.self))
                //where its navigationController == nil. It must be embedded in a Navigation Controller for this to work.")
            return
        }
    ...
    override open func pushViewController(_ viewController: UIViewController, animated: Bool) {
        ...
        if let lastViewController = navigationController.viewControllers.last, !sideMenuManager.menuAllowPushOfSameClassTwice
            && type(of: lastViewController) == type(of: viewController) {
            CATransaction.commit()
            return
        }

        pushMenuController(viewController, animated: animated)
