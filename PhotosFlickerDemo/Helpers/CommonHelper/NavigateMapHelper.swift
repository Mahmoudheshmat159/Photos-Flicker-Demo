...
struct NavigateMap {
    ...
    func openMapForPlace(delegate: UIViewController? = nil) {
        let installedNavigationApps : [String] = ["Apple Maps".localized,"Google Maps".localized, "cancel.lan".localized] // Apple Maps is always installed
    ...
    func openGoogleMapWithLocation(cLocation:CLLocationCoordinate2D) {
        let (lat, lng, _) = (cLocation.latitude ?? 0,cLocation.longitude ?? 0,"")
        if (UIApplication.shared.canOpenURL(URL(string:"comgooglemaps://")!)) {
            UIApplication.shared.open(URL(string:
                "comgooglemaps://?saddr=&daddr=\(lat),\(lng)&directionsmode=driving")!)
        } else {
